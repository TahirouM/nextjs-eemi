...
export default async function SessionDetailPage({
  ...
  const facts = [
    ...
    {
      k: "Niveau",
      v: tLevel(session.activity.level as "all")
    },
  ];
    ...
    <>
      ...
      <div className="mt-4">
        <PageHeader
          ...
          action={
            isCancelled ? (
              <Badge tone="stop">{tStatus(sessionStatusKey[session.status])}</Badge>
            ) : (
              <Badge tone="court">{tStatus(sessionStatusKey[session.status])}</Badge>
            )
          }
        />
      </div>
              ...
              <div className="space-y-3">
                <p className="text-sm">
                  ...
                  <Badge tone={bookingStatusTone[active.status]}>
                    {tStatus(bookingStatusKey[active.status])}
                  </Badge>
                </p>
